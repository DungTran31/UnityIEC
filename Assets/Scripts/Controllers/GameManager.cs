...
public class GameManager : MonoBehaviour
{
    ...
    private IEnumerator AutowinCoroutine()
    {
        ...
        {
            // Perform a game action here
            // For example, make a move or trigger a match
    ...
    {
        while (isAutoLosing && State == eStateGame.GAME_STARTED)
        {
            // Perform a game action here
            // For example, make a move or trigger a match
            m_boardController.MakeLoseMove();

            yield return new WaitForSeconds(0.5f);

            if (m_boardController.IsBottomAreaFull())
            {
                ShowLoseScreen();
                isAutoLosing = false;
            }
